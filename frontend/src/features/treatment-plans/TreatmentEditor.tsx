...
import { fieldClass, buttonClass, surfaceLabels } from './presentation';
export function TreatmentEditor({ patientId, catalog, item, toothNumber, onClose, onSaved }: { patientId: string; catalog: Procedure[]; item?: Treatment; toothNumber?: number; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal onClose={busy ? undefined : onClose} closeOnBackdrop={!busy} closeOnEscape={!busy}>
    <form className="bg-white rounded-2xl p-6 w-full max-w-2xl space-y-4" onSubmit={async e => {
      ...
      <fieldset disabled={busy} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <label className="text-sm">Estado<select className={fieldClass} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as TreatmentStatus, completedAt: e.target.value === 'COMPLETED' ? f.completedAt : null }))}>{Object.entries(treatmentLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></label>
      ...
      <div className="flex justify-end gap-3"><button type="button" disabled={busy} onClick={onClose} className="px-4 py-2 text-slate-600">Cancelar</button><button disabled={busy} className={buttonClass}>{busy ? 'Guardando…' : 'Guardar tratamiento'}</button></div>
    </form>
  </Modal>;
}
