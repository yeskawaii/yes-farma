...
import { fieldClass, buttonClass } from './presentation';
export function ProcedureCatalog({ items, onClose, onSaved }: { items: Procedure[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal onClose={busy ? undefined : onClose}><div className="bg-white w-full max-w-2xl rounded-2xl p-6 space-y-4">
    ...
    <div className="max-h-52 overflow-auto divide-y divide-slate-100">{items.map(p => <button disabled={busy} key={p.id} className="w-full text-left py-3 flex justify-between gap-3" onClick={() => edit(p)}><span>{p.name} {!p.active && '(Inactivo)'}</span><span>{formatMoney(p.defaultPrice)}</span></button>)}{!items.length && <p className="text-slate-500 text-sm">Agrega los procedimientos y precios de tu clínica.</p>}</div>
    <form className="space-y-3" onSubmit={async e => { e.preventDefault(); setBusy(true); setError(''); try { await treatmentApi.saveProcedure({ name, description: description || null, defaultPrice: price, active }, item); await onSaved(); edit(); } catch (e) { setError(e instanceof Error ? e.message : 'Error al guardar'); } finally { setBusy(false); } }}>
      ...
      <div className="flex flex-wrap gap-3"><button disabled={busy} className={buttonClass}>Guardar</button>{item && <button type="button" disabled={busy} onClick={() => edit()}>Nuevo procedimiento</button>}<button type="button" disabled={busy} onClick={onClose} className="ml-auto">Cerrar</button></div>
    </form>
  </div></Modal>;
}
