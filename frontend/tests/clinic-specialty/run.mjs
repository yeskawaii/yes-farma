...
const temporary = await mkdtemp(join(tmpdir(), 'yeskira-specialty-tests-'));
try {
  await build({
    configFile: false, root: resolve('tests/clinic-specialty'), base: './',
  ...
  for (const size of ['1280,800', '500,400']) {
    ...
    const checks = output.match(/<pre id="specialty-test-results"[^>]*>([\s\S]*?)<\/pre>/)?.[1];
    // macOS Chrome may finish the DOM assertions but stall during shutdown.
    if ((result.status !== 0 && result.error?.code !== 'ETIMEDOUT') || !output.includes('data-specialty-tests="passed"')) {
