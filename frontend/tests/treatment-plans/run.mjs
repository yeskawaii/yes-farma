...
const temporary = await mkdtemp(join(tmpdir(), 'yeskira-treatment-tests-'));
try {
  await build({
    configFile: false, root: resolve('tests/treatment-plans'), base: './',
  ...
  for (const size of ['1280,800', '500,400']) {
    ...
    const checks = output.match(/<pre id="treatment-test-results">([\s\S]*?)<\/pre>/)?.[1];
    if (result.status !== 0 || !output.includes('data-treatment-tests="passed"')) {
